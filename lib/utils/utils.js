    ...
    return new Promise(async (resolve) => {
        if (nodes.length) {
            await Promise.all(nodes.map(async ({ name, nodeId, fileId }) => ({ name, fileId, node: await getNode(nodeId, fileId) })))
                .then(async (response) => {
                    if (response[0].node?.err) {
                        error(response[0].node);
                    } else {
            ...
            bin ? error("✖ There are no new icons to import") : resolve(error("✖ There are no new icons to import"));
        }
    })
};
